...
if (!$socket) { 
    ...
    foreach($read as $connect) { //обрабатываем все соединения
		...
		switch ($coin) {
			...
			case "xmr": 
				$creds = (file_get_contents(__DIR__."/monero/1402/monero-wallet-rpc.18083.login"));
				$shell_cmd = "curl -u $creds --digest -X POST http://127.0.0.1:18083/json_rpc -d '{\"jsonrpc\":\"2.0\",\"id\":\"0\",\"method\":\"make_integrated_address\",\"params\":{\"payment_id\":\"$phone_prefix\"}}' -H 'Content-Type: application/json'";
				echo $shell_cmd;
